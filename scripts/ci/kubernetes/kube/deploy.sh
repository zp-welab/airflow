...
# else
#     CONFIGMAP_GIT_SUBPATH=${GIT_SUBPATH:-dags}
#     INIT_DAGS_VOLUME_NAME=airflow-dags-fake
#     POD_AIRFLOW_DAGS_VOLUME_NAME=airflow-dags-git
#     CONFIGMAP_DAGS_FOLDER=/root/airflow/dags/repo/${CONFIGMAP_GIT_SUBPATH}
...
# ${SED_COMMAND} -i "s|{{CONFIGMAP_GIT_SUBPATH}}|$CONFIGMAP_GIT_SUBPATH|g" ${BUILD_DIRNAME}/configmaps.yaml
...
if [[ "${ENV}" ==  "local" ]]; then
  ...
  HOST=$(kubectl get ingress | grep airflow | awk '{print $3}')
  PORT=80
fi
...
for i in {1..30}
do
  HTTP_CODE=$(curl -LI http://${HOST}:${PORT}/health -o /dev/null -w '%{http_code}\n' -sS) || true
